...
public abstract class PullToRefreshAdapterViewBase<T extends AbsListView> extends PullToRefreshBase<T> implements
    ...
    private int getAddedViewCount(){
        boolean isListView = mRefreshableView instanceof ListView;
        int addedCount = 0;
        if(isListView){
            ListView lv = (ListView)mRefreshableView;
            addedCount = lv.getFooterViewsCount()+lv.getHeaderViewsCount();
        }
        return addedCount;
    }
	...
	private boolean isLastItemVisible() {
		...
		if (getAddedViewCount() == 0 && (null == adapter || adapter.isEmpty())) {
			if (DEBUG) {
